...
async function startHttp(port: number): Promise<void> {
  ...
  const httpServer = createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/call-tool") {
      // Auth precedence handled by authenticateRequest:
      //   1. X-API-Key (must match if present)
      //   2. Bearer MI JWT (when MANAGED_IDENTITY_ENABLED)
      //   3. Bearer vault/delegated user token (validated against allow-list)
      const authResult = await authenticateRequest(req);
      if (!authResult.allowed) {
        res.writeHead(401, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: `Unauthorized: ${authResult.error}` }));
        return;
      }
      const userToken = authResult.userToken;
    ...
    if (req.url === "/mcp" || req.url === "/") {
      // Auth precedence handled by authenticateRequest (see auth.ts).
      const authResult = await authenticateRequest(req);
      if (!authResult.allowed) {
        res.writeHead(401, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: `Unauthorized: ${authResult.error}` }));
      ...
      // For MI / X-API-Key auth: userToken is undefined, GraphClient uses SP token
